...
export default function DailyTimePicker({
  DATA,
  setActiveIndex,
}: {
  DATA: number[];
  ...
  const renderItem = ({ item, index }: { item: number; index: number }) => {
    ...
    const backgroundColor = scrollX.interpolate({
      ...
      outputRange: isDarkMode
        ? ["#262626", "#262626", "#00254D", "#262626", "#262626"]
        : ["#F8F8F8", "#F8F8F8", "#E6F2FF", "#F8F8F8", "#F8F8F8"],
      extrapolate: "clamp",
    });

    return (
      <View
        style={{ width: FULL_ITEM_WIDTH, alignItems: "center", marginTop: 10 }}
      >
        <TouchableOpacity
          activeOpacity={0.8}
          onPress={() => {
        ...
        >
          ...
          >
            <Text
              style={{ fontSize: 40 }}
              className={`font-semibold ${isDarkMode ? "text-bg-light" : "text-bg-dark"}`}
            >
              {item}
            </Text>
          </Animated.View>
      ...
      <Animated.FlatList
        ...
        onMomentumScrollEnd={(e) => {
          const index = Math.round(
            e.nativeEvent.contentOffset.x / FULL_ITEM_WIDTH,
          );
