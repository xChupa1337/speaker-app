...
const OffersButton = ({
  ...
  if (bgColor) viewBgColor = bgColor;
  ...
  return (
    ...
    >
      <View>
        <Button
          fullCustomTextClassName="text-label-large text-body-primary-dark"
          className={`px-4 text-label-large ${mainColor ? `bg-[${mainColor}]` : ""}`}
        >
          {buttonTitle}
        </Button>
      </View>
      <View className="text-center max-w-64">
        <Text
          className={`${mainColor ? `text-[${mainColor}]` : "text-primary"} text-title-medium my-2 text-center`}
        >
          {title}
        </Text>
      </View>
